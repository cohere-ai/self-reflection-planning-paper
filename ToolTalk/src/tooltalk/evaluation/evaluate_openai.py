...
class OpenAIPredictor(BaseAPIPredictor):
    ...
    def __init__(self, model, apis_used, disable_docs=False, force_single_hop=False, temperature=None, debug=False):
        self.model = model
        self.api_docs = [api.to_openai_doc(disable_docs) for api in apis_used]
        if force_single_hop:
            raise NotImplementedError(
                "force_single_hop not supported for OpenAI function-calling, as it yields one single call every hop, "
                "which will be unfair. Consider upgrading to parallel function calling first."
            )
    ...
    def predict(self, metadata: dict, conversation_history: dict) -> dict:
        ...
        for turn in conversation_history:
            if turn["role"] == "user" or turn["role"] == "assistant":
                openai_history.append({"role": turn["role"], "content": turn["text"]})
            elif turn["role"] == "api":
                ...
                response_content = {"response": turn["response"], "exception": turn["exception"]}
                openai_history.append(
                    {"role": "function", "name": turn["request"]["api_name"], "content": json.dumps(response_content)}
                )

        openai_response = openai_chat_completion(
            model=self.model, messages=openai_history, functions=self.api_docs, temperature=self.temperature
        )
        ...
        if openai_message.function_call is not None:
            ...
            return {
                ...
                "metadata": {"model": openai_response.model},
            }
        else:
            return {
                "role": "assistant",
...
def get_arg_parser():
    ...
    parser.add_argument(
        "--modes", choices=list(EvalModes), type=str, nargs="+", default=list(EvalModes), help="Evaluation modes"
    )

    return parser


async def main(flags: List[str] = None):
    ...
    for file_name, file_path in tqdm(get_names_and_paths(args.dataset)):
        ...
        if EvalModes.PREDICT in args.modes:
            ...
            predictor_func = OpenAIPredictor(
                model=args.model, apis_used=apis_used, disable_docs=args.disable_documentation
            )
            conversation = await tool_executor.run_conversation(conversation, predictor_func)
