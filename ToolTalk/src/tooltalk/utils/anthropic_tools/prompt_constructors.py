...
def construct_prompt_from_messages(messages):
    ...
    for i, message in enumerate(messages):
        if message["role"] == "system":
            assert i == 0, "System message can only appear as the first message."
            constructed_prompt = message["content"]
            ...
            if (i > 0 and messages[i - 1]["role"] == "user") or i == 0:
                ...
                constructed_prompt = f"{constructed_prompt}\n\n{appendage}"
        elif message["role"] == "tool_outputs":
...
def validate_messages(messages):
    ...
    valid_roles = ["user", "assistant", "tool_inputs", "tool_outputs", "system"]
