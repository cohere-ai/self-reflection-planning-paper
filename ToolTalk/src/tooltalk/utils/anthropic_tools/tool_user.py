...
class ToolUser:
    ...
    def __init__(self, tools, temperature=0, max_retries=3, first_party=True, model="default"):
        ...
        if first_party:
            if model == "default":
                ...
                self.model = model
            # delegating max_retries to backoff
            ...
            # delegating max_retries to backoff
            self.client = AnthropicBedrock(max_retries=0)
        self.current_prompt = None
        self.current_num_retries = 0

    def use_tools(self, messages, verbose=0, execution_mode="manual", max_tokens_to_sample=2000, temperature=1, tool_plan=None):
        """
        ...
        if tool_plan:
            constructed_prompt = constructed_prompt + tool_plan + "\n\nNow, let's execute this plan:\n\n"
        ...
        if completion.stop_reason == "stop_sequence":
            if (
                completion.stop == "</function_calls>"
        ...
        if tool_plan:
            formatted_completion = tool_plan + "\n\nNow, let's execute this plan:\n\n" + formatted_completion
        ...
        while True:
            ...
            self.current_prompt = f"{self.current_prompt}" f"{formatted_completion}\n\n" f"{claude_response}"
    ...
    def _parse_function_calls(self, last_completion, evaluate_function_calls):
        ...
        for invoke_call in invoke_calls["invokes"]:
            ...
            if invalid:
                return {
                    "status": "ERROR",
                    "message": f"Invalid parameters {invalid} for <tool_name>{tool_name}</tool_name>.",
                }
            if missing:
                # TODO: ToolTalk has a lot of tools that come with optional parameters. Even though Claude cannot handle them
                # atm, we don't want it to fail too hard.
                pass
                # return {
                #     "status": "ERROR",
                #     "message": f"Missing required parameters {parameter_names} for <tool_name>{tool_name}</tool_name>.",
                # }
    ...
    # no longer used since we switched to anthropic.AnthropicBedrock
    ...
    def _function_calls_valid_format_and_invoke_extraction(last_completion):
        ...
        for invoke_string in invoke_strings:
            ...
            tags = re.findall(
                r"<.*?>", parameters[0].replace("<parameters>", "").replace("</parameters>", ""), re.DOTALL
            )
            if len(tags) % 2 != 0:
            ...
            for i in range(0, len(tags), 2):
                ...
                match = re.search(rf"{opening_tag}(.*?){closing_tag}", parameters[0], re.DOTALL)
                if match is None:
                    return {
                        "status": False,
                        "reason": "Error extracting parameter value from <parameters></parameters> tags.",
                    }
                parameters_with_values.append((opening_tag[1:-1], match.group(1)))
    ...
    @staticmethod
    def _convert_value(value, type_str: str):
        ...
        """
        type_str = type_str.replace("List", "list").split("[")[0]
